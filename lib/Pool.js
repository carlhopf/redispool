...
const Log = require('jslib/utils/Log');
...
function Pool(options) {
	...
	this._maxConnections = options.maxConnections || 8;
...
Pool.prototype._popOrCreate = function (cb) {
	if (this._clients.length > 0) {
		...
		Log.debug(tag, 'pop() create new client');
		this._create(cb);
	}
};
...
Pool.prototype.pop = function (cb) {
	...
	this._popQueue.push(function (client, cbQueue) {
		var to = setTimeout(function () {
			throw new Error('client not returned within ' + MS_RETURN_POP);
		}, MS_RETURN_POP);
...
Pool.prototype._create = function (cb) {
	...
	var client = Client.create(
		...
		() => {
			// success
			this._count++;
			Log.debug(this.tag, 'connect success, count ' + this._count);
