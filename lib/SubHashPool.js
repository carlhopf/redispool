...
class SubHashPool {
	...
	subscribe (channel, fnjson, fnready) {
		...
		// cancel subscription
		// - remove onjson listener
		// - remove onready listener
		var cancel = (cb) => {
			if (cancelled) {
				return;
			}
			...
			// already was ready, only call readyfn with err
			if (onready === true) {
				process.nextTick(() => fnready('subscribe cancelled'));
			}

			// if set to true meanwhile, ready listener already called
			if (this.onready[channel] !== true) {
				var index = onready.indexOf(fnready);
				if (index === -1) throw new Error('fnready not found');
				onready.splice(index, 1);
		...
		if (!onjson) {
			...
			client.subscribe(channel, (err) => {
				...
				onready.slice().forEach((fn) => fn(err, cancel));
			});
		}

		// is ready (if cancelled before nextTick, do not run)
		if (onready === true) {
